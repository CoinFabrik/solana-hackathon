    ...
    <div className="w-full max-w-md mx-auto">
      ...
      {programsManager?.programs?.length ? (
        programsManager?.programs.map((program, index) => {
          if(program.networkId == programsManager.networkInfo.selectedNetwork) return (
          <div key={program.address.toString()} className="flex justify-between my-2">
            <span>{program.idl.name}</span>
            <Button variant="link" onClick={() => handleRemove(program.address)}>
              <X className="h-4 w-4" />
            </Button>
          </div>
          )}
